...
7. **Lifecycle and retention。** 顺序固定为：
   ...
   acknowledgment 前成功执行 `worker-release`。Worker branch、Git objects 与 anchor 必须
   保持 recoverable 直到 settlement。只有 mapping 与 verification evidence durable、
   result accepted/rejected 后，Lead 才删除 local anchor 以及 local/remote
...
Root 先 create/bind Run，再 create Task，并通过 version-matched `worker-start` 与显式
base selection 把 Execution Packet 交给 Execution Lead。Every supervised writable
Root-to-Execution-Lead dispatch MUST be launched through `orca orchestration worker-start`;
low-level `worktree create` plus `orchestration dispatch --inject` does not register the
Lead in Orca's `worker-*` lifecycle registry, so the Root cannot settle it with
`worker-release`. 等待期间使用 long structured `check --wait` windows，不做固定
sleep/poll loop、频繁 terminal read 或 step-by-step direction。
...
- writable Worker 按 §5.4 必须成功执行 `worker-release`；或
- 仅对 non-writable Worker，立即把同一个 agent terminal 转移到 follow-up Dispatch；或
- 仅对 non-writable Worker，在用户明确要求时执行 `worker-retain`。
...
acknowledgment 前成功执行 `worker-release`；Worker Git objects、branch 与 anchor 保持
